...
router.get('/episodes', (req, res) => {
    db.Episode.find()
        ...
        .catch(err => {
            console.log(err);
            res.status(500).send({ message: 'Database Error: This error usually is a result of an error in the code or program rather than with the web server itself.' });
        });
}
);
    ...
    db.Episode.findById(req.params.id)
        .then(episode => {
            if (episode) {
                ...
                res.status(404).send({ message: 'Episode not located. This error is usually a problem with the URL link.' })
            }
        })
        .catch(err => {
            console.log(err);
